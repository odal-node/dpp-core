//! Cross-crate integration test: AAS mapping across every product group.
//!
//! `build_aas_from_passport` (dpp-aas) is the primary AAS entry
//! point — it dispatches each product group's `ProductGroupData` to a dedicated submodel
//! mapper. This test feeds a fully-populated passport for **every** product group
...
//!
//! It complements the per-product group E2E tests (battery, textile) by guaranteeing
//! no product group mapper silently regresses or panics, and that the AAS submodel
...
/// product group-submodel `idShort`.
fn all_product_group_cases() -> Vec<(ProductGroup, ProductGroupData, &'static str, &'static str)> {
    vec![
        (
        ...
        (
            ...
            ProductGroupData::Textile(Box::new(textile_data())),
            "1.1.0",
            "TextileMaterialDeclaration",
        ),
        (
            ProductGroup::Steel,
            ProductGroupData::Steel(steel_data()),
            "1.0.0",
            "SteelProductData",
        ),
        (
            ProductGroup::Construction,
            ProductGroupData::Construction(construction_data()),
            "1.0.0",
            "ConstructionProductData",
        ),
        (
            ProductGroup::Tyre,
            ProductGroupData::Tyre(tyre_data()),
            "1.0.0",
            "TyreProductData",
        ),
        (
            ProductGroup::Toy,
            ProductGroupData::Toy(toy_data()),
            "1.0.0",
            "ToyProductData",
        ),
        (
            ProductGroup::Aluminium,
            ProductGroupData::Aluminium(aluminium_data()),
            "1.0.0",
            "AluminiumProductData",
        ),
        (
            ProductGroup::Furniture,
            ProductGroupData::Furniture(furniture_data()),
            "1.0.0",
        ...
        (
            ...
            // fail `only_in_force_product_groups_carry_a_typed_mapper`.
            ProductGroup::Mattress,
            ProductGroupData::Mattress(mattress_data()),
            "1.0.0",
            "ProductGroupData",
        ),
        (
            ProductGroup::Detergent,
            ProductGroupData::Detergent(detergent_data()),
            "1.0.0",
...
fn every_product_group_produces_a_valid_aas_shell() {
    for (product_group, data, version, _id_short) in all_product_group_cases() {
        ...
        // ProductGroupData::product group() must report the variant's own discriminant.
        assert_eq!(
            data.product_group().catalog_key(),
            key,
            "ProductGroupData::product_group() must match its variant"
        );
        ...
        for core in [
            ...
            assert!(
                ...
                "{core} submodel missing for product_group {key}"
            );
        }

        // The product group submodel exists and carries at least its mandatory fields.
        let product_group_submodel = submodels
            ...
            .expect("a product_group-specific submodel is present");
        assert!(
            !product_group_submodel.submodel_elements.is_empty(),
            "product_group submodel for {key} should not be empty"
        );
    }
}

/// An unrecognised product group's fields never reach the AAS output, for any audience.
///
/// The catalog has no descriptor for such a product group, so no field of it is
...
#[test]
fn unknown_product_group_fields_never_reach_the_aas_output() {
    let spacecraft = || {
        ProductGroupData::other(serde_json::json!({
    ...
    // Every audience, not just Public: an unmodelled product group has no field policy
    ...
    for audience in [
        ...
        let passport = base(
            ProductGroup::Other("spacecraft".into()),
            spacecraft(),
            "1.0.0",
        );
        ...
        assert_eq!(
            submodels.len(),
            6,
            "{audience:?}: the product_group submodel is still present"
        );
        let generic = submodels
            .iter()
            .find(|s| s.id_short == "ProductGroupData")
            .expect("generic ProductGroupData submodel present");

        assert!(
            generic.submodel_elements.is_empty(),
            "{audience:?}: an unmodelled product_group's fields reached the AAS output: {:?}",
        ...
        // Not vacuous: the product group is still identified, via ProductIdentification.
        assert!(
            rendered.contains("spacecraft"),
            "{audience:?}: the product_group is no longer identified at all"
        );
    }
}
...
/// that condition would empty every product group submodel and every masking assertion
/// above would still pass. This is the guard against that.
#[test]
fn a_known_product_group_is_not_caught_by_the_unknown_product_group_backstop() {
    let passport = base(
        ProductGroup::Electronics,
        ProductGroupData::Electronics(electronics_data()),
        "1.0.0",
    ...
    assert!(
        !product_group_submodel.submodel_elements.is_empty(),
        "a catalogued product_group was blanket-redacted by the unknown-product_group backstop"
    );
}

#[test]
fn passport_without_product_group_data_has_five_core_submodels() {
...
#[test]
fn aas_submodel_templates_resolve_for_known_product_groups() {
    let battery = product_group_submodel_template("battery").expect("battery template exists");
    assert_eq!(battery.product_group_key, "battery");

    let textile = product_group_submodel_template("textile").expect("textile template exists");
    assert!(textile.is_placeholder());

    // Unknown product group → no template.
    assert!(product_group_submodel_template("spacecraft").is_none());
...
#[test]
fn no_product_group_template_yet_names_a_ratified_third_party_standard() {
    let ratified: Vec<&str> = dpp_domain::ProductGroupCatalog::new()
        .all()
        .iter()
        .filter_map(|d| product_group_submodel_template(d.key.as_str()))
        .filter(|t| !t.is_placeholder())
        .map(|t| t.product_group_key)
        .collect();
...
fn a_template_version_matches_its_identifier() {
    for template in dpp_domain::ProductGroupCatalog::new()
        .all()
        .iter()
        .filter_map(|d| product_group_submodel_template(d.key.as_str()))
    {
...
/// Every catalog product group must be exercised through the AAS path somewhere.
///
/// Without this, adding a product group to the catalog and forgetting to extend
/// `all_product_group_cases` leaves that product group's AAS mapping untested — silently,
/// because the loop above simply iterates one case fewer.
#[test]
fn every_catalog_product_group_has_an_aas_case() {
    ...
    let covered: Vec<String> = all_product_group_cases()
        .iter()
    ...
    for d in catalog.all().iter() {
        ...
        assert!(
            ...
            "catalog product_group '{key}' has no case in all_product_group_cases() — its AAS              mapping is untested"
        );
    }
}
...
fn every_emitted_semantic_id_is_ours_or_provenanced() {
    ...
    for (product_group, data, version, _) in all_product_group_cases() {
        ...
        assert!(
            !found.is_empty(),
            "product_group '{key}' emitted no semanticIds at all — the walker is broken, \
             not the output"
        );
        ...
        for (path, id) in &found {
            assert!(
                is_permitted(id),
                "product_group '{key}' emits unprovenanced semanticId '{id}' at '{path}'. \
    ...
    // its own hardcoded semanticId, and a gate that only walked `product_groups/` is
...
/// Every product group's submodel template identifier, including the ones no passport
...
#[test]
fn every_product_group_template_semantic_id_is_ours_or_provenanced() {
    let catalog = dpp_domain::catalog::ProductGroupCatalog::new();

    for descriptor in catalog.all().iter() {
        let key = descriptor.key.as_str();
        let Some(template) = product_group_submodel_template(key) else {
            continue;
        };
        assert!(
            is_permitted(template.semantic_id),
            "product_group template '{key}' carries unprovenanced semanticId \
             '{}'",
            template.semantic_id
        );
    }
...
/// The one product group permitted a typed mapper while still `provisional` in the
...
/// it. Recorded here, deliberately, rather than assumed — one product group at a time.
const PILOT_CARVE_OUT: &str = "textile";

/// A typed mapper exists **only** for a product group whose act is in force, plus the
...
/// a dedicated submodel for *every* catalog product group, which meant a newly
/// announced product group could not be added as catalog data alone — it needed Rust
...
/// rendering of a product group whose ratified template has not been published.
///
...
#[test]
fn only_in_force_product_groups_carry_a_typed_mapper() {
    ...
    for (product_group, data, version, _id_short) in all_product_group_cases() {
        ...
        let product_group_submodel = submodels
            ...
            .expect("a product_group submodel is always present");

        // The generic projection is the one named for the field it renders;
        // a typed mapper names its own submodel template.
        let is_typed = product_group_submodel.id_short != "ProductGroupData";
        let may_be_typed = !instruments.determinable_for(&key).is_empty() || key == PILOT_CARVE_OUT;

        assert_eq!(
            is_typed, may_be_typed,
            "product_group '{key}': typed mapper present = {is_typed}, but permitted = {may_be_typed}. \
             A provisional product_group must render generically; an in-force one must not."
        );
    }
}
...
fn the_idshort_rule_diverges_between_revisions() {
    let submodel_named = |name: &str| {
        serde_json::json!({"submodels": [{
            "id": "urn:odal-node:dpp:x:product_group-data",
            "idShort": name,
            "modelType": "Submodel"
        }]})
    };
...
fn no_element_carries_a_member_outside_the_metamodel() {
        ...
        match node {
            serde_json::Value::Object(map) => {
                if let Some(class) = map.get("modelType").and_then(serde_json::Value::as_str) {
                    ...
                    if allowed.is_empty() {
                        unknown.push(format!(
                            "product_group '{key}': `{path}` declares modelType '{class}', which no \
                        ...
                        for member in map.keys() {
                            if !allowed.contains(member.as_str()) {
                                unknown.push(format!(
                                    "product_group '{key}': `{path}` is a {class} carrying '{member}', \
    ...
    // Guard against the walk silently covering nothing: every product group emits a
...
/// Every product group's public Environment validates against IDTA's own schema.
#[test]
fn every_product_group_environment_is_schema_valid() {
    let mut checked = 0usize;
    for (product_group, data, version, _) in all_product_group_cases() {
        let key = product_group.catalog_key().to_owned();
        let passport = base(product_group, data, version);
        ...
        assert_valid_aas(
            &document,
            &format!("the public Environment for product_group '{key}'"),
        );
        checked += 1;
    }
    assert!(checked > 0, "the gate asserted nothing");
...
#[test]
fn the_generic_product_group_environment_is_schema_valid() {
    let other = ProductGroupData::other(serde_json::json!({
        "productGroup": "spacecraft",
        "thrustKn": 500.0,
        "reusable": true,
        "stageCount": 2
...
fn pinned(
    ...
    let fixed = "2026-01-01T00:00:00Z"
        .parse::<DateTime<Utc>>()
        .expect("a date");
    let mut passport = base(product_group, data, version);
...
///
/// It is absent from `all_product_group_cases` (its mapping is exercised by the
...
fn battery_case() -> (ProductGroup, ProductGroupData, &'static str) {
    ...
    (ProductGroup::Battery, data, "1.0.0")
}
...
fn environment_fixture_path(product_group_key: &str) -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("fixtures/aas/environments")
        .join(format!("{product_group_key}.json"))
}
...
fn committed_environments_match_what_the_mappers_produce() {
    ...
    let mut cases: Vec<(ProductGroup, ProductGroupData, &str)> = all_product_group_cases()
        .into_iter()
        .map(|(s, d, v, _)| (s, d, v))
        .collect();
        ...
        for (field, class) in &descriptor.disclosure {
            ...
            assert!(
                ...
                "the committed Environment for '{key}' carries '{field}', which \
                 that product_group classifies as {class:?}"
            );
        }
        checked += 1;
    }
...
/// No field a product group's catalog entry classifies non-public appears in a public
/// AAS projection — asserted field-by-field from the catalog, for every product group
...
/// stops it becoming one, and it is driven from the product group manifests so that
...
fn public_aas_projection_emits_no_non_public_field() {
    ...
    for (product_group, data, version, _) in all_product_group_cases() {
        ...
        for field in &non_public {
            assert!(
                !emitted.contains(field),
                "non-public field '{field}' of product_group '{key}' appears in the PUBLIC AAS projection"
            );
        }
    }
    ...
    assert!(
        ...
        "guard: at least one product_group must declare non-public fields, or this gate proves nothing"
    );
}
