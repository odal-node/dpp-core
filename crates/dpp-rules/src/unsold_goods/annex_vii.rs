...
/// scope (apparel & clothing accessories, or footwear). Prefix match against
/// the two headings above — `commodity_code` is expected to be a bare 6/8/10
/// digit string (as produced by `CommodityCode::as_str()` in `dpp-domain`).
#[must_use]
pub fn is_within_annex_vii_scope(commodity_code: &str) -> bool {
    APPAREL_AND_CLOTHING_ACCESSORIES_PREFIXES
        .iter()
        .chain(FOOTWEAR_PREFIXES)
        .any(|prefix| commodity_code.starts_with(prefix))
}

#[cfg(test)]
mod tests {
    use super::is_within_annex_vii_scope;
