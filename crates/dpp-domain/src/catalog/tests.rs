...
fn battery_descriptor_is_complete() {
    ...
    // Current version is v2.5.0, which makes batteryType required and closed
    // (Annex VI Part A point 2 via Annex XIII point 1(a)). Older versions stay
    // registered so passports already validated against them remain verifiable.
    assert_eq!(battery.current_schema_version, "2.5.0");
