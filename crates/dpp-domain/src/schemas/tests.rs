...
fn registry_loads_all_embedded_schemas() {
    ...
    // battery 1.0 + 2.0 + 2.1 + 2.2 + 2.3 + 2.4, textile 1.0 + 1.1 + 1.2, unsold-goods 1.0,
...
#[test]
fn latest_battery_returns_v2_4() {
    let reg = VersionedSchemaRegistry::new();
    let (version, _json) = reg.latest("battery").expect("battery schema exists");
    assert_eq!(*version, "2.4.0".parse::<Version>().unwrap());
}
...
fn register_or_replace_existing_returns_true() {
    ...
    let replaced = reg
        ...
        .unwrap();
    assert!(replaced);
    assert_eq!(reg.len(), 25); // count unchanged
    assert!(
...
fn unregister_embedded_schema_does_nothing() {
    ...
    assert!(!removed);
    assert_eq!(reg.len(), 25); // still there
}
