//! Validation helpers for product group payload lists.
//!
//! The cross-field rule logic lives once in `dpp-rules` (shared with the Wasm
//! product group plugins). These thin adapters map the domain structs onto that crate's
//! primitive inputs, preserving the public API and error messages.

use crate::domain::product_group::data::{FibreEntry, SurfactantEntry, SvhcSubstance};
