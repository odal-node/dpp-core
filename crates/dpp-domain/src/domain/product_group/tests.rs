//! Redaction, validation, and serde round-trip tests for product group data.

use super::*;
use crate::catalog::{ProductGroupCatalog, ProductGroupDescriptor};
...
fn minimal_battery_data() -> ProductGroupData {
...
// ── G-8: per-product group Rust type ↔ JSON schema conformance ─────────────────
//
...
fn state_of_health_round_trips_both_annex_vii_parameter_sets() {
    use crate::domain::product_group::StateOfHealth;
...
fn expected_lifetime_round_trips_with_illustrative_harmful_events() {
    use crate::domain::product_group::{ExpectedLifetime, HarmfulEvents};
...
fn part_b_unconditional_items_are_required() {
    use crate::domain::product_group::ExpectedLifetime;
