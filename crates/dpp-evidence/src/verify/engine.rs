...
use crate::dossier::{DossierV1, compute_content_hashes};
use crate::jws::{resolve_public_key, verify_jws_content};

use super::report::{CheckResult, CheckStatus, VerificationReport, VerifyMode};
...
///
/// For typed callers that already hold a `DossierV1` (e.g. the engine's own
/// round-trip test, which never serialises to bytes). File/network consumers
/// should use [`verify_dossier_json`] instead — it additionally runs the
/// `input_fidelity` check, which needs the original raw bytes.
pub fn verify_dossier(dossier: &DossierV1, mode: VerifyMode) -> VerificationReport {
    ...
    let trust_anchor_note = match mode {
        VerifyMode::Embedded => format!(
            "trust anchored to embedded key snapshot dated {}",
            dossier.manifest.created_at
        ),
        VerifyMode::Online => "trust anchored to live-fetched DID documents".to_string(),
    };
...
/// Parse raw dossier bytes and verify them — the entry point real consumers
/// (CLI, wasm) should use.
///
...
///   type this crate doesn't make strict) parses fine but is silently
...
pub fn verify_dossier_json(
    ...
    let mut report = verify_dossier(&dossier, mode);
...
fn input_fidelity_check(raw: &serde_json::Value, parsed: &DossierV1) -> CheckResult {
    let status = (|| -> Result<CheckStatus, String> {
        let raw_bytes = serde_jcs::to_vec(raw).map_err(|e| format!("canonicalising input: {e}"))?;
        let reparsed_value = serde_json::to_value(parsed)
            .map_err(|e| format!("reserialising parsed dossier: {e}"))?;
        let reparsed_bytes = serde_jcs::to_vec(&reparsed_value)
...
mod tests {
    ...
    use crate::dossier::{DossierManifest, SignedLayer};
    use base64::Engine;
    use chrono::Utc;
    ...
    fn clean_dossier_verifies_fully() {
        ...
        let report = verify_dossier(&dossier, VerifyMode::Embedded);
    ...
    fn tampered_transfer_signature_flips_only_transfer_chain() {
        ...
        let clean_report = verify_dossier(&dossier, VerifyMode::Embedded);
    ...
    fn clean_json_round_trips_through_verify_dossier_json() {
        ...
        let report = verify_dossier_json(&bytes, VerifyMode::Embedded).expect("parses");
        assert!(report.all_verified(), "{report:?}");
    ...
    fn unknown_top_level_field_is_a_parse_error() {
        ...
        let err = verify_dossier_json(&bytes, VerifyMode::Embedded)
    ...
    fn unknown_field_nested_in_a_tolerant_type_fails_input_fidelity() {
        ...
        let report = verify_dossier_json(&bytes, VerifyMode::Embedded)
    ...
    fn malformed_json_is_a_parse_error() {
        let err = verify_dossier_json(b"not json", VerifyMode::Embedded).unwrap_err();
        assert!(matches!(err, DossierParseError::Json(_)));
    }
}
