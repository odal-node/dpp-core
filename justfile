...
#
# The private-material scan is no longer a step here: it is a test in
# `dpp-tests`, so `test` runs it. That is why the gate needs no interpreter —
# it used to shell out to Python for the one tripwire that needs no library at
# all, while the oracles were kept in their own workflows precisely so the gate
# would not acquire a toolchain. What it catches is still the only failure here
# that cannot be undone after a release.
